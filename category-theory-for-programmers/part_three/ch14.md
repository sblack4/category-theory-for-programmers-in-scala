...
As you may remember, algebraic theories can be described using monads — in particular [algebras for monads](ch14.md#algebras-for-monads). It should be no surprise then that there is a connection between Lawvere theories and monads.

First, let’s see how a Lawvere theory induces a monad. It does it through an [adjunction](ch14.md#freeforgetful-adjunctions) between a forgetful functor and a free functor. The forgetful functor `U` assigns a set to each model. This set is given by evaluating the functor M from `Mod(L, Set)` at the object `1` in **L**.
...
Together, the forgetful and the free functor define a [monad](ch14.md#monads-categorically) `T = U∘F` on **Set**. Thus every Lawvere theory generates a monad.

It turns out that the category of [algebras for this monad](ch14.md#algebras-for-monads) is equivalent to the category of models.
...
Here, the coend starts as the disjoint sum of sets `an× L(n, 1)` over all `n`s. The identifications can be generated by expressing the [coend as a coequilizer](ch14.md#ends-and-coends). We start with an off-diagonal term `an× L(m, 1)`. To get to the diagonal, we can apply a morphism `f :: m -> n` either to the first or the second component of the product. The two results are then identified.
...
Next: [Monads, Monoids, and Categories](ch14.md#monads-monoids-and-categories/).
